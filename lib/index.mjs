...
function registerMcpTools(ctx, mode, gate, filterFor = () => NO_TOOL_FILTER) {
	...
	*/
	const bindAgentScope = (agent) => {
		...
		}, `claude-compat: mcp mounts of ${agent.id}`);
	};
	const stopAll = async () => {
		const pending = [];
...
async function apply(ctx, config = {}) {
	...
	*/
	const warnFiltersWithoutEffect = (next) => {
		...
		const configured = Object.entries(next.mcpTools).filter(([, value]) => filterHidesAnything(parseMcpToolFilter(value))).map(([key]) => key);
		if (configured.length === 0) return;
		ctx.logger.warn(`claude-compat: MCP loading is "eager", so the tool filters for ${configured.join(", ")} have no effect. eager mounts every enabled row through mcp-client, which registers all discovered tools; select the "dynamic" or "lazy" loading mode to apply these filters.`);
	};
...
//#endregion
export { ClaudeCompatMcp, Config, MCP_LOADING_MODES, PLUGIN_ID, admits, apply, assertServerName, filterHidesAnything, filterMcpTools, inject, instructionsSource, isInstructionsSource, mcpEntryConfig, mcpRowKey, name, parseMcpLoadingMode, parseMcpToolFilter, registerMcpTools, specFromEntryConfig, toolRuleEntries };
