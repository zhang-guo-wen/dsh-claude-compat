...
- **按需加载 MCP。** 停用的服务器不占任何开销。模型可调用 `mcp_list`、`mcp_load`、`mcp_unload`，**只为当前会话**
  ...
  连接按会话隔离：不同会话各起一份；会话结束后，它为这台服务器建立的连接会自动关闭。
- **MCP 工具过滤。** 在 MCP 的编辑弹窗里列出这台服务器提供的方法，**默认全部勾选**；取消勾选的方法不会进上下文 ——
  既不列出也不能调用。需要通配符时也可以手写 `context-injection.mcpTools`，见下。
...
**需要通配符时手写：**规则存在 `context-injection` 设置命名空间的 `mcpTools` 字段里，键是行标识
（`preset:<preset id>:<serverName>`，与 MCP 描述同键）：
